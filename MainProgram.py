...
class MainWindow(QMainWindow):
    ...
    def signalconnect(self):
        ...
        self.ui.label_12.clicked.connect(self.export)
    ...
    def export(self):
        ...
        if len(self.data)>30:
            ...
            selected_data = self.data[::step]

        self.main_win = export.ExportWindow(selected_data)
        self.main_win.show()
    ...
    def tabel_info_show(self, locations, clses, confs, path=None):
        ...
        for location, cls, conf in zip(locations, clses, confs):
            ...
            # 数据记录
            self.data.insert(0,[item_id.text(),item_path.text(),False if len(str(Config.CH_names[cls]))==4 else True,
                              item_conf.text(),item_location.text()])
